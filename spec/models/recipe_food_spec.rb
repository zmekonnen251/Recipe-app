...
RSpec.describe RecipeFood, type: :model do
  describe 'validations' do
    @user = User.create(name: 'test', email: 'testemail@test.com', password: 'testtest',
                        password_confirmation: 'testtest')
    @food = Food.create(name: 'test', measurement_unit: 'kg', price: 1, user: @user)
    @recipe = Recipe.create(name: 'test', description: 'test', cooking_time: 1, preparation_time: 1, public: true,
                            user: @user)
